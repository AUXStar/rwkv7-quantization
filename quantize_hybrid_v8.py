#!/usr/bin/env python3
"""Hybrid v8 quantization: AWQ + per-block optimal clip ratio.

Iteration 8 — combine AWQ channel scaling with per-block clip ratio search:
- FFN key.weight:   NVFP4 W4A16 with AWQ scaling + per-block optimal clip ratio
- FFN value.weight: BF16 (unchanged)
- Attention r/k/v/output.weight: FP8 E4M3 (W8A16)

Combines the strengths of v6 and v7:
  v6 (clip ratio only): PPL delta 0.0014 (great), Top-1 97.76% (bad)
  v7 (AWQ only):       PPL delta 0.0097 (ok),   Top-1 98.38% (better)
  v8 (AWQ + clip):     Goal: low PPL + high Top-1

Algorithm:
  1. Compute AWQ scaling: s_k = (act_k^alpha) / (w_k^(1-alpha)), normalized
  2. Apply AWQ: W' = W * s (protect important channels)
  3. Per-tensor scale from W'
  4. For each block of 16 elements, search clip ratios [0.6-1.0]
     to find the scale that minimizes per-block MSE
  5. Pack FP4 pairs into uint8

Output: /home/njzy/model/rwkv7-g1h-2.9b-hybrid-v8.pth
"""
import torch
import os
import time
...
def quantize_nvfp4_awq_clip(w, awq_scale, block_size=16, clip_ratios=None, device='cuda'):
    """NVFP4 quantization with AWQ scaling + per-block optimal clip ratio.

    Steps:
    1. Apply AWQ scaling: W' = W * s
    2. Compute per-tensor scale from W' amax
    3. For each block, search clip ratios to minimize per-block MSE
    4. Pack FP4 pairs into uint8
    """
    if clip_ratios is None:
        clip_ratios = [0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 1.00]

    # Apply AWQ scaling on GPU
    w_orig = w.to(device=device)
    s = awq_scale.to(device=device)
    w_awq = w_orig.float() * s.float().unsqueeze(0)  # [N, K] scaled

    N, K = w.shape
    n_blocks = K // block_size

    # Per-tensor scale from AWQ-scaled weights
    amax = w_awq.abs().max()
    ...
    # Reshape to blocks
    w_blocks = w_awq.view(N, n_blocks, block_size)
...
def quantize_nvfp4_default(w, block_size=16, device='cuda'):
    """Default NVFP4 quantization (clip ratio = 1.0, no AWQ). For comparison."""
    w_orig = w.to(device=device)
    N, K = w.shape
    w_f32 = w_orig.float()
    n_blocks = K // block_size
...
def verify_quantization(w_orig, packed, bs, ts, awq_scale, device='cuda'):
    """Verify AWQ+clip NVFP4 quantization by computing reconstruction error."""
...
def main():
    ...
    parser.add_argument("--output", default="/home/njzy/model/rwkv7-g1h-2.9b-hybrid-v8.pth")
    ...
    for key in list(z.keys()):
        ...
        # FFN key -> NVFP4 with AWQ + clip ratio
        if ".ffn.key.weight" in key and ".nf4_b_scale" not in key and ".nvfp4_t_scale" not in key:
            ...
            # Quantize with AWQ + clip ratio
            w_packed, w_bs, w_ts, _ = quantize_nvfp4_awq_clip(w, awq_s, block_size=16)
            ...
            if nvfp4_count <= 2:
                print(f"  AWQ+clip+NVFP4 {key}: [{N},{K}] "
    ...
    print(f"    AWQ+clip+NVFP4 (FFN key):  {nvfp4_count} tensors")
    ...
    if args.verify and first_key_orig is not None:
        print(f"\n=== Verification: AWQ+clip NVFP4 reconstruction ===")
        ...
        # v8: AWQ + clip
        mse_v8, max_err_v8, cos_v8 = verify_quantization(
            first_key_orig, packed, bs, ts, awq_s)
        print(f"  v8 AWQ+clip: MSE={mse_v8:.10f}, MaxErr={max_err_v8:.6f}, CosSim={cos_v8:.10f}")

        # Default NVFP4 (no AWQ, no clip) for comparison
        ...
        print(f"  Improvement:  MSE {(1 - mse_v8/mse_def)*100:.1f}%, "
              f"MaxErr {(1 - max_err_v8/max_err_def)*100:.1f}%, "
              f"CosSim +{cos_v8 - cos_def:.8f}")

    # Metadata
    meta = {
        "v": 8,
        "r": [
            [0, L-1, 4, 2],  # ffn_key: nvfp4 + awq + clip
            [0, L-1, 1, 1],  # att_receptance: fp8
            [0, L-1, 2, 1],  # att_key: fp8
            [0, L-1, 3, 1],  # att_value: fp8
            [0, L-1, 6, 1],  # att_output: fp8
        ],
        "s": {"blk": 16, "sd": "fp8e4m3", "td": "fp32", "awq": True, "clip": True, "alpha": args.alpha},
        "n": [
        ...
        "desc": f"Hybrid v8: FFN key AWQ+clip+NVFP4 (alpha={args.alpha}) + FFN value BF16 + Attention FP8 W8A16",
    }
    z["meta"] = meta
