#!/usr/bin/env python3
"""Benchmark: Full Weight-Only Quantization (NVFP4 W4A16 + FP8 W8A16).

Both FFN paths use weight-only quantization — no activation quantization at all:
- FFN key: NVFP4 W4A16 (dequant weight → BF16, FP16 GEMM)
- FFN value: FP8 W8A16 (dequant weight → FP16, FP16 GEMM)

This eliminates ALL activation quantization error from FFN, isolating the
pure weight quantization error of NVFP4 (16 levels) + FP8 (256 levels).

Comparison points:
- #2 W4A4 (NVFP4 W4A4 + FP8 W8A8): PPL 0.0176, Top-1 96.90%, decode 34 tok/s
- W4A16 only (NVFP4 W4A16 + FP8 W8A8): PPL 0.0138, Top-1 98.14%, decode 8 tok/s
- #3 FP8-Att W8A16 (attention only): PPL 0.0009, Top-1 99.71%, decode 39 tok/s
"""
import sys
...
# ============================================================================
# Configuration: enable both W4A16 and W8A16
# ============================================================================
MIXED_MODEL = "/home/njzy/model/rwkv7-g1h-2.9b-mixed-nvfp4-fp8.pth"
EVAL_DIR = "/home/njzy/test/eval_tmp"

engine.NVFP4_W4A16 = True   # NVFP4 key: weight-only (no FP4 activation quant)
engine.FP8_W8A16 = True     # FP8 value: weight-only (no FP8 activation quant)
...
print("=" * 80)
print("  Benchmark: Full Weight-Only (NVFP4 W4A16 + FP8 W8A16)")
print("=" * 80)
...
print("\n  Generating logits with W4A16+W8A16 model...")
logits_446 = generate_logits(model, tokens_446, "w4a16_w8a16_446")
logits_2100 = generate_logits(model, tokens_2100, "w4a16_w8a16_2100")

# Save logits
torch.save(logits_446, f"{EVAL_DIR}/logits_w4a16_w8a16_446.pt")
torch.save(logits_2100, f"{EVAL_DIR}/logits_w4a16_w8a16_2100.pt")

# Compare with original
r_446 = compare_logits(f"{EVAL_DIR}/logits_orig_446.pt", logits_446, f"{EVAL_DIR}/test_446.json", "W4A16+W8A16 446")
r_2100 = compare_logits(f"{EVAL_DIR}/logits_orig_2100.pt", logits_2100, f"{EVAL_DIR}/test_2100.json", "W4A16+W8A16 2100")
...
print(f"\n{'='*100}")
print(f"  Full Comparison: All FFN Quantization Modes")
print(f"{'='*100}")
print(f"  {'Mode':<30} {'PPL delta':<12} {'Top-1%':<10} {'CE delta':<12} {'Prefill':<10} {'Decode':<10}")
print(f"  {'-'*85}")
...
if os.path.exists(w4a4_path):
    with open(w4a4_path) as f:
        w4a4 = json.load(f)
    print(f"  {'W4A4+W8A8 (fused)':<30} {w4a4['quality_2100']['ppl_delta']:<12.4f} {w4a4['quality_2100']['top1_agree']*100:<10.2f} {w4a4['quality_2100']['ce_delta']:<12.6f} {w4a4['prefill']['2100']['toks_per_s']:<10.0f} {w4a4['decode']['toks_per_s']:<10.0f}")
...
if os.path.exists(w4a16_path):
    with open(w4a16_path) as f:
        w4a16 = json.load(f)
    print(f"  {'W4A16+W8A8':<30} {w4a16['quality_2100']['ppl_delta']:<12.4f} {w4a16['quality_2100']['top1_agree']*100:<10.2f} {w4a16['quality_2100']['ce_delta']:<12.6f} {w4a16['prefill']['2100']['toks_per_s']:<10.0f} {w4a16['decode']['toks_per_s']:<10.0f}")

# W4A16+W8A16 (this)
print(f"  {'W4A16+W8A16 (this)':<30} {r_2100['ppl_delta']:<12.4f} {r_2100['top1_agree']*100:<10.2f} {r_2100['ce_delta']:<12.6f} {prefill_results[2100]['toks_per_s']:<10.0f} {decode_r['toks_per_s']:<10.0f}")
...
if os.path.exists(att_path):
    with open(att_path) as f:
        att = json.load(f)
    print(f"  {'FP8-Att W8A16 (ref)':<30} {att['quality_2100']['ppl_delta']:<12.4f} {att['quality_2100']['top1_agree']*100:<10.2f} {att['quality_2100']['ce_delta']:<12.6f} {att['prefill']['2100']['toks_per_s']:<10.0f} {att['decode']['toks_per_s']:<10.0f}")
...
with open(f"{EVAL_DIR}/bench_w4a16_w8a16_results.json", "w") as f:
    json.dump(results, f, indent=2)
print(f"\n  Results saved to {EVAL_DIR}/bench_w4a16_w8a16_results.json")
