#!/usr/bin/env python3
"""Combined quantization: FFN (NVFP4 key + FP8 value) + Attention FP8 (W8A16).

Combines experiments #2 and #3 into a single model:
- FFN key.weight: NVFP4 (4-bit, block scaled) — maximum compression for largest tensor
- FFN value.weight: FP8 E4M3 (8-bit, per-tensor) — better precision for value path
- Attention r/k/v/output.weight: FP8 E4M3 (W8A16) — high precision, low overhead

Output: /home/njzy/model/rwkv7-g1h-2.9b-combined-nvfp4-fp8-attfp8.pth
"""
import torch
...
def main():
    ...
    parser.add_argument("--output", default="/home/njzy/model/rwkv7-g1h-2.9b-combined-nvfp4-fp8-attfp8.pth")
    ...
    print(f"    FP8   (FFN value):    {ffn_fp8_count} tensors")
    ...
    meta = {
        ...
        "desc": "Combined: FFN key NVFP4 + FFN value FP8 + Attention r/k/v/output FP8 W8A16",
    }
    z["meta"] = meta
