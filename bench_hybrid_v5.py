#!/usr/bin/env python3
"""Benchmark: Hybrid v5 — NVFP4 block_size=8 FFN key + BF16 value + FP8 W8A16 att."""
import sys
...
HYBRID_MODEL = "/home/njzy/model/rwkv7-g1h-2.9b-hybrid-v5.pth"
...
print("=" * 80)
print("  Benchmark: Hybrid v5 (NVFP4 bs=8 W4A16 key + BF16 val + FP8 att)")
print("=" * 80)
...
for k in ["blocks.0.ffn.key.weight", "blocks.0.ffn.value.weight"]:
    ...
    if isinstance(w, dict):
        ...
        print(f"  {k}: qtype={w['qtype']}, bs={list(bs.shape)}, ts={ts_info}")
    else:
...
print("\n  Generating logits...")
logits_446 = generate_logits(model, tokens_446, "v5_446")
logits_2100 = generate_logits(model, tokens_2100, "v5_2100")
torch.save(logits_446, f"{EVAL_DIR}/logits_hybrid_v5_446.pt")
torch.save(logits_2100, f"{EVAL_DIR}/logits_hybrid_v5_2100.pt")

r_446 = compare_logits(f"{EVAL_DIR}/logits_orig_446.pt", logits_446, f"{EVAL_DIR}/test_446.json", "Hybrid-v5 446")
r_2100 = compare_logits(f"{EVAL_DIR}/logits_orig_2100.pt", logits_2100, f"{EVAL_DIR}/test_2100.json", "Hybrid-v5 2100")
...
print(f"{'='*100}")
print(f"  {'Mode':<50} {'PPL delta':<12} {'Top-1%':<10} {'Prefill':<10} {'Decode':<10}")
print(f"  {'-'*95}")

for name, path in [
    ("W4A4+W8A8 (fused)", f"{EVAL_DIR}/bench_fused_results.json"),
    ("W4A16+W8A16 (per-tensor, both w-o)", f"{EVAL_DIR}/bench_w4a16_w8a16_results.json"),
    ("FP8-Att W8A16 (att only)", f"{EVAL_DIR}/bench_att_fp8_results.json"),
    ("v3 (per-tensor NVFP4 key+BF16 val+FP8 att)", f"{EVAL_DIR}/bench_hybrid_v3_results.json"),
]:
    if os.path.exists(path):
        with open(path) as f:
            d = json.load(f)
        print(f"  {name:<50} {d['quality_2100']['ppl_delta']:<12.4f} {d['quality_2100']['top1_agree']*100:<10.2f} {d['prefill']['2100']['toks_per_s']:<10.0f} {d['decode']['toks_per_s']:<10.0f}")

print(f"  {'v5 (NVFP4 bs=8 key+BF16 val+FP8 att)':<50} {r_2100['ppl_delta']:<12.4f} {r_2100['top1_agree']*100:<10.2f} {prefill_results[2100]['toks_per_s']:<10.0f} {decode_r['toks_per_s']:<10.0f}")
...
with open(f"{EVAL_DIR}/bench_hybrid_v5_results.json", "w") as f:
    json.dump(results, f, indent=2)
print(f"\n  Results saved to {EVAL_DIR}/bench_hybrid_v5_results.json")
