#!/usr/bin/env python3
"""Benchmark: Hybrid v4 — Per-channel NVFP4 W4A16 key + BF16 value + FP8 W8A16 att.

Iteration 4: per-channel tensor scale for NVFP4 to break the 98.43% Top-1 barrier.
- FFN key:   NVFP4 W4A16 with per-channel tensor scale (one scale per output row)
- FFN value: BF16 (no quantization)
- Attention: FP8 W8A16 (dequant weight -> FP16, FP16 GEMM)

Comparison:
- v3 (per-tensor NVFP4 key): Top-1 98.43%, PPL 0.0106, decode 9 tok/s
- FP8-Att only:              Top-1 99.71%, PPL 0.0009, decode 39 tok/s
"""
import sys
...
print("=" * 80)
print("  Benchmark: Hybrid v4 (Per-channel NVFP4 W4A16 key + BF16 val + FP8 att)")
print("=" * 80)
...
for k in ["blocks.0.ffn.key.weight", "blocks.0.ffn.value.weight"]:
    ...
    if isinstance(w, dict):
        ...
        print(f"  {k}: qtype={w['qtype']}, ts={ts_info}")
    else:
...
print("\n  Generating logits with Hybrid v4 model...")
logits_446 = generate_logits(model, tokens_446, "hybrid_v4_446")
logits_2100 = generate_logits(model, tokens_2100, "hybrid_v4_2100")

# Save logits
torch.save(logits_446, f"{EVAL_DIR}/logits_hybrid_v4_446.pt")
torch.save(logits_2100, f"{EVAL_DIR}/logits_hybrid_v4_2100.pt")

# Compare with original
r_446 = compare_logits(f"{EVAL_DIR}/logits_orig_446.pt", logits_446, f"{EVAL_DIR}/test_446.json", "Hybrid-v4 446")
r_2100 = compare_logits(f"{EVAL_DIR}/logits_orig_2100.pt", logits_2100, f"{EVAL_DIR}/test_2100.json", "Hybrid-v4 2100")
...
print(f"{'='*100}")
print(f"  {'Mode':<45} {'PPL delta':<12} {'Top-1%':<10} {'Prefill':<10} {'Decode':<10}")
print(f"  {'-'*90}")
...
if os.path.exists(w4a4_path):
    with open(w4a4_path) as f:
        w4a4 = json.load(f)
    print(f"  {'W4A4+W8A8 (fused kernel)':<45} {w4a4['quality_2100']['ppl_delta']:<12.4f} {w4a4['quality_2100']['top1_agree']*100:<10.2f} {w4a4['prefill']['2100']['toks_per_s']:<10.0f} {w4a4['decode']['toks_per_s']:<10.0f}")
...
if os.path.exists(w8a16_path):
    with open(w8a16_path) as f:
        w8a16 = json.load(f)
    print(f"  {'W4A16+W8A16 (per-tensor, both weight-only)':<45} {w8a16['quality_2100']['ppl_delta']:<12.4f} {w8a16['quality_2100']['top1_agree']*100:<10.2f} {w8a16['prefill']['2100']['toks_per_s']:<10.0f} {w8a16['decode']['toks_per_s']:<10.0f}")
...
if os.path.exists(att_path):
    with open(att_path) as f:
        att = json.load(f)
    print(f"  {'FP8-Att W8A16 (att only)':<45} {att['quality_2100']['ppl_delta']:<12.4f} {att['quality_2100']['top1_agree']*100:<10.2f} {att['prefill']['2100']['toks_per_s']:<10.0f} {att['decode']['toks_per_s']:<10.0f}")

# v3
v3_path = f"{EVAL_DIR}/bench_hybrid_v3_results.json"
if os.path.exists(v3_path):
    with open(v3_path) as f:
        v3 = json.load(f)
    print(f"  {'v3 (per-tensor NVFP4 key+BF16 val+FP8 att)':<45} {v3['quality_2100']['ppl_delta']:<12.4f} {v3['quality_2100']['top1_agree']*100:<10.2f} {v3['prefill']['2100']['toks_per_s']:<10.0f} {v3['decode']['toks_per_s']:<10.0f}")

# This run (v4)
print(f"  {'v4 (per-channel NVFP4 key+BF16 val+FP8 att)':<45} {r_2100['ppl_delta']:<12.4f} {r_2100['top1_agree']*100:<10.2f} {prefill_results[2100]['toks_per_s']:<10.0f} {decode_r['toks_per_s']:<10.0f}")

# Save results
results = {
    "model": HYBRID_MODEL,
    "mode": "Hybrid-v4-perchannel",
...
with open(f"{EVAL_DIR}/bench_hybrid_v4_results.json", "w") as f:
    json.dump(results, f, indent=2)
print(f"\n  Results saved to {EVAL_DIR}/bench_hybrid_v4_results.json")
