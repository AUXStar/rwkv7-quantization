...
def main() -> None:
    ...
    parser.add_argument("--no-cuda-graph", action="store_true", help="disable CUDA Graph, use plain forward")
    ...
    for item in args.cases.replace(",", " ").split():
        ...
        bench_case(model, B, T, args.warmup, args.iters, args.profile_range, args.no_cuda_graph)
...
class RWKV7:
    def __init__(self) -> None:
        ...
        for key in list(z.keys()):
            if key == "emb.weight" and emb_cpu is not None:
                continue
            if key.endswith(".fp8_scale") or key.endswith(".awq_scale"):
                continue
            if key not in z:
                continue
    ...
    def linear_ffn_down(self, x: torch.Tensor, weight: torch.Tensor, rows: int) -> torch.Tensor:
        # FP8 quantized weight: route to FP8 GEMM
        if isinstance(weight, dict):
            if FUSED_GEMM and rows <= FUSED_M_MAX:
                return linear_quantized_fused(x, weight, out_dtype=DTYPE)
            return linear_quantized(x, weight, out_dtype=DTYPE)
    ...
    def linear_orig_layout(self, x: torch.Tensor, weight: torch.Tensor, path: PathConfig, group: str) -> torch.Tensor:
        # FP8 quantized weight: route to FP8 GEMM
        if isinstance(weight, dict):
            if FUSED_GEMM and path.rows <= FUSED_M_MAX:
                return linear_quantized_fused(x, weight, out_dtype=DTYPE)
            return linear_quantized(x, weight, out_dtype=DTYPE)
...
def bench_case(model: RWKV7, B: int, T: int, warmup: int, iters: int, profile_range: bool, no_cuda_graph: bool = False) -> None:
    ...
    if pp_enabled():
        if no_cuda_graph:
            # Plain forward without CUDA Graph
            times = []
            if profile_range:
        ...
        # Original PP path with CUDA Graph
        segments = pp_segments()
        stage_inputs = []
        stage_vfirst = []
        stage_outputs = []
    ...
    # Original non-PP path with CUDA Graph
    graph = torch.cuda.CUDAGraph()
